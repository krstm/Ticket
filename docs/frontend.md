...
- **JavaScript entry**: Frontend/src/main.js registers Alpine and exports helper factories—most notably `ticketDetailsPage`, which powers the ticket detail interactions.

## 2. Build & Verification Workflow
1. `npm install` (first run).
2. `npm run dev` for live reloads inside MVC.
3. `npm run build` to refresh `wwwroot/dist/` before committing or publishing.
...
- `ticketDetailsPage` exposes actor fields (name, email, actorType) but **never persists them client-side**; users provide the info per action until real authentication ships.
