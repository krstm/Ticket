...
| Interfaces/Services | Service contracts (ITicketService, ICategoryService, IReportingService, INotificationService). Repository contracts were deleted—services talk to ApplicationDbContext directly. |
...
- **Future search/messaging headroom:** The domain-event pipeline already emits TicketCreatedEvent, TicketStatusChangedEvent, and TicketResolvedEvent. Adding mail/webhook handlers later will be a matter of registering new INotificationHandler<T> implementations—no changes to TicketService needed.
...
1. **Pipeline:** UseSerilogRequestLogging › CorrelationIdMiddleware › ExceptionHandlingMiddleware › HTTPS/static files › routing › fixed-window rate limiting (global + "mutations") › authorization.
...
3. **Controllers:** Slim—each controller calls the relevant service and returns DTOs. Status-specific behaviour (e.g., If-Match header parsing) lives inside controllers so services remain HTTP-agnostic.
4. **Services:**
...
| UpdateAsync | Row-version guard, sanitization, normalization, timestamp updates. Does **not** touch histories—only domain events do. |
...
- ApplyFilters — centralizes search terms, category/status/priority filters, requester/recipient matches, created/due date ranges, and TicketSearchScope semantics (Title-only vs full content).
- ApplySorting — whitelisted sort expressions (CreatedAt, Priority, Status, CategoryName, DueAt) with deterministic tie-breakers (Id).
- ProjectToSummary — projection expression so EF performs server-side DTO shaping (no extra AutoMapper call on search).
...
With this architecture, every behaviour (validation, normalization, paging, domain events, logging, security, testing) is documented and traceable, enabling future contributors—or LLM copilots—to extend the platform safely.
